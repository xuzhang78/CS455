...
//						max_dc_ac_length[dc_number][15]=65535;
...
int	KINGJPG::get_pic()
{
	...
	after_anti_entropy=new short int[looptime*64*x_size*y_size];
	after_anti_dct=new short int[looptime*64*x_size*y_size];
	now=*(pixel+place);
	int ret;
	for (ynow=0;ynow<y_size;ynow++){
		...
		if (ret<0)
			return -1;
		anti_quantizer(after_anti_entropy);
		for (int i=0;i<looptime;i++)
			jpeg_IDCT(after_anti_entropy+i*64,after_anti_dct+i*64);
...
int KINGJPG::anti_quantizer(short int *after_anti_entropy)
{
	int looptime;
	...
	for (int tablenum=0;tablenum<looptime-2;tablenum++)	//first
		for (ii=0;ii<64;ii++)
		*(after_anti_entropy+tablenum*64+ii)=*(after_anti_entropy+tablenum*64+ii)*quantize[usedquan[0]][ii];
	for (ii=0;ii<64;ii++)		//second
		*(after_anti_entropy+(looptime-2)*64+ii)=*(after_anti_entropy+(looptime-2)*64+ii)*quantize[usedquan[1]][ii];
	for (ii=0;ii<64;ii++)		//third
		*(after_anti_entropy+(looptime-1)*64+ii)=*(after_anti_entropy+(looptime-1)*64+ii)*quantize[usedquan[2]][ii];
...
int		KINGJPG::color(int xnow,int ynow,short int *before_dct)
{
	...
	for (int y=0;y<8*yscale;y++)
		for (int x=0;x<8*xscale;x++){
			...
			place4=y*16+x;
			*(before_dct+place3)=-0.5+0.299*(*(point+place1*3))+0.587*(*(point+place1*3+1))+0.114*(*(point+place1*3+2));
			*(cb+place4) = -0.5+128 - 0.15847*(*(point+place1*3))-0.33126*(*(point+place1*3+1))+0.5*(*(point+place1*3+2));
			*(cr+place4) = -0.5+128 + 0.5*(*(point+place1*3))-0.41869*(*(point+place1*3+1))-0.08131*(*(point+place1*3+2));
		}
